...
envirise <- function(grid, dat = NULL, xy, env, cutoff){
  if (! is.null(dat)){
    xy  <- dat[, xy]
    env <- dat[, env]
  }
  ...
  lyrL <- lapply(lvls, function(lvl){
    ...
    rasterize(xyMat, grid, fun = length)
  }  )
  lyrs <- rast(lyrL)
  names(lyrs) <- lvls
  ...
  lyrsNoNA <- classify(lyrs, reclass)
  prop <- lyrs / sum(lyrsNoNA) # convert to proportion
  clasfy <- rbind(c(0, cutoff, 0), # binarise
                  c(cutoff, 1, 1))
  # values = cutoff classified as above the cutoff (left-closed interval)
  mjtys <- classify(prop, clasfy,
                    include.lowest = TRUE,
                    right = FALSE)
  values(grid) <- NA # ensure blank starting raster
  for (l in 1:length(lvls)){
    grid[mjtys[[l]]==1] <- l
  }

  # change NA cells to indet. if they do have occurrence data
  # but no one environmental class exceeds cutoff
  anydat <- rasterize(data.matrix(xy), grid)
  anydat <- classify(anydat, cbind(1, length(lvls)+1))
  grid <- cover(grid, anydat)

  lvls <- c(lvls, 'indet.')
  attrs <- data.frame('num' = 1:length(lvls))
