...
```{r}
sampSumry <- sapply(circOccs, sdsumry, 
                    taxVar = 'genus',
                    collections = 'collection_no',
                    xy = c('cellLng','cellLat')
                    )
sampSumry <- data.frame( t(sampSumry) )
colnames(sampSumry) <- colnames(samp1)[1:ncol(sampSumry)] # fewer columns without rarefaction
quantile(sampSumry$nTax, c(0.25, 0.5, 0.75))
```
