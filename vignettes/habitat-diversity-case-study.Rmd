...
cellMat <- cells(rPrj, prjColl) 
collSilur$cell <- cellMat[,'cell']
xyPrj <- xyFromCell(rPrj, collSilur$cell)
xyCell <- c('cellX','cellY')
collSilur[, xyCell] <- xyPrj
```
...
# rasterise to same grid as collections data
...
# determine environment of each occurrence directly where possible
# but in some places it won't be clear from associated 'environment' field -
# in those cases, infer the predominant enviro in the cell based on collections

occSilur$bathCell <- occSilur$bath <- extract(bathRast, prjOccs)[['mainClass']]
# these variables will inherit class as factor - keep it this way
# for correct ordering of factor levels when inferring preferences below
...
# Classify environmental preferences 
...
pref <- function(env, cutoff){
  ...
  if (sum(envTbl[1:2])==0) { 'unk' } else {
    envTbl <- envTbl[1:2]/sum(envTbl[1:2])
    if (any(envTbl[1:2] > cutoff)) {
...
for (tax in taxa){
 ...
 occSilur$pref[taxBool] <- pref(envOccs, cutoff = 0.8)
}

# remove duplicate taxon-site occurrences to save on memory
# this must be done AFTER establishing preferences:
# duplicate spatial occurrences could have unique enviro information associated
occSilur <- uniqify(occSilur, taxVar = 'genus', xy = xyCell)
```
...
Don't weight? if not enough pool cells to matter much
```{r subsample, error=TRUE}
deep <- occSilur[ occSilur$pref == 'deep', ]
prox <- occSilur[ occSilur$pref == 'prox.', ]
# how many taxa specialise in each?
length(unique(deep$genus))
length(unique(prox$genus))
# cutoff of 0.8 gives 69 deep, 52 shallow
# cutoff of 0.7 gives 94 deep, 61 shallow

# double-checking: species tagged with unknown preference totally lack env data:
  # View(occSilur[ occSilur$pref == 'unk', ]) # 19 genera with unk affinity
...
sampP <- cookies(dat = prox, 
                 ...
                 weight = TRUE,
                 crs = prj, output = 'full')
# we have to expand the subsample bounds to get a good sample of sites!
# (or settle for fewer sites per subsample)
r <- 1500
...
library(iterators)
library(doParallel)
library(beepr) # for notification when code finishes running

# test case:
# tmp <- subsampRange(df = sampD[[1]], taxa = taxa, taxCol = 'genus',
...
rngD <- foreach(df = sampD, .inorder = FALSE, .packages = 'divvy') %dopar%
  subsampRange(df = df, taxa = taxa, taxCol = 'genus', 
               coordCols = xyCell, crs = prj)

rngP <- foreach(df = sampP, .inorder = FALSE, .packages = 'divvy') %dopar%
  subsampRange(df = df, taxa = taxa, taxCol = 'genus', 
               coordCols = xyCell, crs = prj)
...
beep()
#        user      system     elapsed 
# 0.018500000 0.002833333 0.867666667 
```

```{r load range size data, include = FALSE}
save(rngD, rngP, file = 'range-data-weighted.RData')
load('range-data-weighted.RData')
```
...
# test case
atrypa <- taxRange('Atrypa', rngD)
round(atrypa, 2)

# TODO apply over all taxa of each PE class in subsamples of preferred enviro
```
...
```{r var corrs}
# TODO redo this whole chunk

metaD <- sdsumry(dat = sampD, taxVar = 'genus', xy = xyCell, crs = prj)
metaP <- sdsumry(dat = sampP, taxVar = 'genus', xy = xyCell, crs = prj)
metaD$occAvg <- metaD$nOcc / metaD$nTax
metaP$occAvg <- metaP$nOcc / metaP$nTax

# NB looks like this SHOULD average mean pair dist and MST for all species in a subsample, 
# then add that info to meta df's as 2 new columns with the suffix 'Tax'
# TODO fix this before any of the lines here or later chunks will work
