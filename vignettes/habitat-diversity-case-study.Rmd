...
rWorld <- rast()
prj <- 'EPSG:8857'
rPrj <- project(rWorld, prj, res = 200000)
...
occSilur$bath <- extract(bathRast, prjOccs)[['mainClass']] |> as.character()
deep <- occSilur[occSilur$bath == 'deep',]
prox <- occSilur[occSilur$bath == 'prox.',]
```

A quick inspection reveals roughly equal numbers of occurrences in each environment. However, restricting consideration to places containing brachiopod occurrences, there are twice as many shallow as deep cells.
...
# TODO also subsample all cells - for taxa with no preference/across depths
```
# Range size calculation

```{r fcns for custom range size}
subsampRange <- function(taxa, df, taxCol, coordCols, crs){
  # remove multiple occs of a single taxon within a cell, for accurate occ count
# df <- uniqify(df, taxVar = taxCol, xy = coordCols) # unnecessary, but possibly speedier?
...
beep()
#       user     system    elapsed 
# 0.03016667 0.02083333 6.64933333 
```
