...
anyDat <- rasterize( data.matrix(occSilur[, xyCell]), rPrj)
bathMask <- mask(bathRast, anyDat)
# bathMask <- mask(bathRast, prjOccs) # fatal error, maybe a memory issue
freq(bathMask)
```
...
subsampRange <- function(taxa, df, taxCol, coordCols, crs){
  ...
  df <- uniqify(df, taxVar = taxCol, xy = coordCols)
  ...
  # also count unique occurrences
  rngDf$occ <- NA
  freq <- table(df[ ,taxCol ])
  occPos <- match(names(freq), taxa)
  rngDf$occ[occPos] <- as.numeric(freq)
  return(rngDf)
}
...
beep()
 #       user      system     elapsed 
 # 0.01666667  0.01583333 47.82000000 
```

```{r load range size data, include = FALSE}
# even in parallel, the run-time is 1 hr. (58 min elapsed)
# save(rngD, rngP, file = 'DP-range-data-xtaxon.Rdata')
# save(rngD, rngP, file = 'DP-range-data.RData')
# save(rngD, rngP, file = 'DP-range-data-weighted.RData')
# save(rngD, rngP, file = 'DP-range-data-clustr.RData')
load('DP-range-data-xtaxon.RData')
```
